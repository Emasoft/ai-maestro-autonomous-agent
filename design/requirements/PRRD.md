...
- **S4.2** — The README `**Version**:` line and the persona `**Plugin**: … vX.Y.Z` line MUST equal `plugin.json` `version`. `publish.py`'s `check_version_consistency()` enforces this, and the live Step-9 bumper `language_bump_version()` keeps all four sources (plugin.json, pyproject.toml, README, persona) in sync on every release. (v4.2, 2026-08-18: the rule previously credited `do_bump()`, a wrapper with zero production callers that was removed under TRDD-J48IO8F3 — the v1.3.0 incident proved patches aimed at it never reach a release.)
