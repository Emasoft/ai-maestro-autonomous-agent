...
def ensure_cliff_config(root: Path) -> None:
    ...
    This literal is the FALLBACK copy, used only when the repo has no
    cliff.toml. It MUST stay byte-equivalent to the committed cliff.toml —
    two copies of the same config are two chances to drift, and this one
    silently won the day cliff.toml went missing. It previously carried the
    OLD indented body template (the TRDD-R3JRZURT bug) long after cliff.toml
    was fixed, so a repo regenerating from this fallback would have gotten the
    indented-heading changelog back. Any edit to cliff.toml must be mirrored
    here; `tests/test_changelog_generation.py` asserts the no-indentation
    invariant against BOTH copies.

    Written as a raw string so the TOML regexes (`\\(`, `\\s`) and the Tera
    tags are stored literally — the previous non-raw form needed four
    backslashes to express two and was a standing escaping hazard.
    """
    cliff_toml = root / "cliff.toml"
    if cliff_toml.exists():
        return
    default = r'''# git-cliff config — auto-generated by publish.py
# https://git-cliff.org/docs/configuration
#
# The body template is deliberately NOT indented: Tera preserves the leading
# whitespace of a `\`-continued line, so canon's indented form emits indented
# headings and bullets into CHANGELOG.md (TRDD-R3JRZURT, commit b326c65).
...
- {% if commit.breaking %}[**breaking**] {% endif %}{% if commit.scope %}**{{ commit.scope }}:** {% endif %}{{ commit.message | upper_first }}{% if commit.id %} ({{ commit.id | truncate(length=7, end="") }}){% endif %}{% endfor %}
{% endfor %}
"""
trim = true
# Non-empty on purpose: git-cliff emits a newline after the last release body,
# so an empty footer leaves CHANGELOG.md ending on a blank line (markdownlint
# MD012).
footer = """
---
...
commit_preprocessors = [
  { pattern = '\((\w+\s)?#([0-9]+)\)', replace = "([#${2}](https://github.com/Emasoft/ai-maestro-autonomous-agent/issues/${2}))" },
  { pattern = '\s+$', replace = "" },
]
commit_parsers = [
...
Examples:
  ...
  %(prog)s --gate               # pre-push gate: every validation step,
    ...
    if info.has_kind(ProjectKind.CLAUDE_PLUGIN):
        ...
        run([
            "uvx", "--from", "git+https://github.com/Emasoft/claude-plugins-validation@v3.2.0",
    ...
    for name in (
        ...
        "cliff.toml",
        ".gitignore",
        # README.md carries the `**Version**: X.Y.Z` badge bumped in Step 9 by
        # update_readme_version(). Without staging it, the badge bump is written
        # but never committed — the release ships a stale README version and the
        # tree is left dirty (the next run aborts at Step 1). Staging an unchanged
        # README is a harmless no-op.
        "README.md",
    ...
    for py_path in plugin_root.rglob("*.py"):
        ...
        try:
            ...
            pass
    # Stage any agent persona (agents/**/*.md) whose `**Plugin**: <name> vX.Y.Z`
    # display string was bumped to the new version by update_persona_versions()
    # in Step 9. Same rationale as README.md above: bump-without-stage leaves the
    # persona version stale in the release commit and the tree dirty afterward.
    agents_dir = plugin_root / "agents"
    if agents_dir.is_dir():
        persona_marker = re.compile(r'\*\*Plugin\*\*:\s*\S+\s+v' + re.escape(new_version))
        for md_path in agents_dir.rglob("*.md"):
            try:
                if persona_marker.search(md_path.read_text(encoding="utf-8")):
